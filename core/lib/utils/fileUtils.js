...
class FileUtils {
   ...
   * @returns {Promise<void>}
   */
  static async zipDirectory(sourceDir, zipPath) {
    try {
   ...
   * @returns {Promise<void>}
   */
  static async unzipFile(zipPath, destDir) {
    try {
      const AdmZip = require('adm-zip');
   ...
   */
  static async searchTextInFiles(dir, searchTerm, filePattern = '*') {
    const files = await this.findFiles(dir, filePattern);
    const results = [];
